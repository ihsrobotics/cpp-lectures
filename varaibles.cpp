...
int main()
{
    ...
    // these r system dependent (hence int64_t and int32_t)
    int a = 10;

    // integer division
    int zero = 1 / 3; // truncates the value

    float actuallyzero = 1 / 3; // truncates the value since 1 / 3 = 0, then that gets typecasted to 0.0

    // casting means that we tell the compiler to use that datatype
    // instead of the original datatype
    float notzero = static_cast<float>(1) / static_cast<float>(3);
